...
const audienceFilter = require('../../services/broadcasts/audienceFilter.service');
  ...
  try {
    ...
    const audiences = await Promise.all(result.rows.map(async (audience) => {
      if (audience.is_static) {
        ...
        // Dynamic: calculate based on filter criteria (using shared service)
        audience.contacts_count = await audienceFilter.calculateDynamicAudienceCount(userId, audience.filter_criteria);
      }
      return audience;
    }));
...
async function getAudience(req, res) {
  try {
    ...
    if (audience.is_static) {
      ...
      // Calculate dynamic count based on filter (using shared service)
      audience.contacts_count = await audienceFilter.calculateDynamicAudienceCount(userId, audience.filter_criteria);
    }
...
async function getAudienceContacts(req, res) {
  try {
    ...
    if (audience.is_static) {
      ...
      // Dynamic audience - filter contacts using shared service
      const { whereClause, params } = audienceFilter.buildFilterQuery(userId, audience.filter_criteria);
